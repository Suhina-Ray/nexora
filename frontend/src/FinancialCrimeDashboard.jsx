...
export default function FinancialCrimeDashboard() {
  ...
  useEffect(() => {
    ...
    for (const t of transactions) {
      ...
      if (!linkMap.has(key))
        linkMap.set(key, { source: t.from, target: t.to, count: 0, amount: 0 });
  ...
  useEffect(() => {
    ...
    function drawArrow(ctx, x1, y1, x2, y2, targetR, lw, alpha, color) {
      ...
      ctx.moveTo(ex, ey);
      ctx.lineTo(
        ex - hl * Math.cos(angle - Math.PI / 7),
        ey - hl * Math.sin(angle - Math.PI / 7)
      );
      ctx.lineTo(
        ex - hl * Math.cos(angle + Math.PI / 7),
        ey - hl * Math.sin(angle + Math.PI / 7)
    ...
    function frame() {
      ...
      if (ctx.createConicGradient && nodes.length) {
        ...
        ctx.strokeStyle = "rgba(240,180,41,0.04)";
        ctx.lineWidth = 1;
        ctx.stroke();
      }
  ...
  const highRiskCount = useMemo(
    () => accounts.filter((a) => a.risk_score > HIGH_RISK_THRESHOLD).length,
    [accounts]
  );
  const avgRisk = useMemo(
    () =>
      accounts.length
        ? accounts.reduce((s, a) => s + a.risk_score, 0) / accounts.length
        : 0,
    [accounts]
  );
  ...
  const ledgerTxns = useMemo(() => {
    ...
    let rows = transactions.map((t) => {
      ...
      const maxRisk = Math.max(
        fromAcc?.risk_score ?? 0,
        toAcc?.risk_score ?? 0
      );
      return { ...t, fromAcc, toAcc, maxRisk };
    });

    if (ledgerRiskFilter === "high")
      rows = rows.filter((t) => t.maxRisk > HIGH_RISK_THRESHOLD);
    ...
    rows.sort((a, b) => {
      if (key === "timestamp")
        return mult * (new Date(a.timestamp) - new Date(b.timestamp));
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div
              ...
              style={{ fontSize: 18, lineHeight: 1.2, fontWeight: 800 }}
            >
              Transaction Network
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              position: "relative",
              background: showAlerts ? "#1c2330" : "none",
              border: "1px solid #1c2330",
              borderRadius: 6,
              padding: "6px 8px",
              cursor: "pointer",
          ...
          >
            <Bell
              size={15}
              style={{ color: alertCount > 0 ? "#f0b429" : "#64748b" }}
            />
            {alertCount > 0 && (
              <span
                style={{
          ...
          {/* Filter toggle */}
          <button
            onClick={() => setFilterMode(filterMode === "all" ? "high" : "all")}
            style={{
              background:
                filterMode === "high" ? "rgba(239,68,68,0.15)" : "none",
              border: `1px solid ${
                filterMode === "high" ? "#ef444466" : "#1c2330"
              }`,
              borderRadius: 6,
              padding: "6px 10px",
              cursor: "pointer",
              color: filterMode === "high" ? "#ef4444" : "#64748b",
              fontSize: 10,
              fontFamily: "inherit",
              letterSpacing: "0.1em",
              display: "flex",
          ...
          <div style={{ overflowY: "auto", maxHeight: 360 }}>
            ...
            {alerts.map((alert) => (
              <div
                ...
                onClick={() => {
                  setSelectedId(alert.account.id);
                  setShowAlerts(false);
                }}
                style={{
                  padding: "10px 14px",
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div
                    style={{ fontSize: 11, color: "#e6edf3", lineHeight: 1.4 }}
                  >
                    {alert.message}
                  </div>
                ...
                >
                  {Math.round(alert.account.risk_score)}
                </span>
              </div>
            ))}
          </div>
              ...
              >
                ...
                <div
                  style={{
                    ...
                    borderRadius: 3,
                    background:
                      "linear-gradient(90deg, #2dd36f, #eab308, #ef4444)",
                    marginBottom: 4,
                  }}
                />
                <div
                ...
                <div style={{ fontSize: 9, color: "#334155", marginTop: 6 }}>
                  size = risk · center = highest concern
                </div>
                <div
                  style={{
                    marginTop: 8,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span
                    style={{
                      ...
                      height: 8,
                      borderRadius: "50%",
                      background: riskColor(hoveredNode.risk_score),
                      display: "inline-block",
                    }}
                  />
                  <span style={{ fontSize: 11, color: "#94a3b8" }}>
                    Risk {Math.round(hoveredNode.risk_score)}
                  </span>
                  ...
                  {loading && !error && (
                    <>
                      ...
                      <div style={{ fontSize: 13, color: "#94a3b8" }}>
                        Initialising investigation system...
                      </div>
                    </>
                  )}
                  {error && (
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: "100%",
                        background: riskColor(acc.risk_score),
                        borderRadius: 2,
                      }}
                    />
                  </div>
          ...
          >
            ...
            >
              ...
              <button
                onClick={() =>
                  setLedgerRiskFilter(
                    ledgerRiskFilter === "all" ? "high" : "all"
                  )
                }
                style={{
                  background:
                    ledgerRiskFilter === "high"
                      ? "rgba(239,68,68,0.15)"
                      : "none",
                  border: `1px solid ${
                    ledgerRiskFilter === "high" ? "#ef444466" : "#1c2330"
                  }`,
                  borderRadius: 6,
                  padding: "7px 12px",
                  cursor: "pointer",
                  color: ledgerRiskFilter === "high" ? "#ef4444" : "#64748b",
                  fontSize: 10,
                  fontFamily: "inherit",
                  letterSpacing: "0.1em",
                  display: "flex",
            ...
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "1.1fr 1fr 1fr 0.7fr 0.9fr 1fr",
                gap: 8,
                padding: "8px 12px",
                fontSize: 9,
                letterSpacing: "0.15em",
            ...
            >
              ...
              <SortableHeader
                label="Amount"
                active={ledgerSort.key === "amount"}
                dir={ledgerSort.dir}
                onClick={() => toggleLedgerSort("amount")}
              />
              <SortableHeader
                label="Risk"
                active={ledgerSort.key === "risk"}
                dir={ledgerSort.dir}
                onClick={() => toggleLedgerSort("risk")}
              />
              <SortableHeader
                label="Timestamp"
                active={ledgerSort.key === "timestamp"}
                dir={ledgerSort.dir}
                onClick={() => toggleLedgerSort("timestamp")}
              />
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              {ledgerTxns.slice(0, 300).map((t) => (
                ...
                >
                  ...
                  >
                    <ArrowDownLeft
                      size={11}
                      style={{ color: "#2dd36f", flexShrink: 0 }}
                    />
                    <span
                      style={{
                        color: "#cbd5e1",
              ...
              {ledgerTxns.length > 300 && (
                ...
                >
                  SHOWING FIRST 300 OF {ledgerTxns.length} — REFINE SEARCH TO
                  NARROW
                </div>
              )}
            </div>
          ...
          >
            ...
            >
              <div>
                ...
                <div
                  className="font-display"
                  style={{ fontSize: 17, marginTop: 2, fontWeight: 700 }}
                >
                  {selectedAccount.name}
                </div>
                <div style={{ marginTop: 6 }}>
                  <RiskBadge score={selectedAccount.risk_score} />
                </div>
              </div>
              <button
            ...
            >
              ...
              >
                ...
                <span
                  ...
                  style={{
                    fontSize: 32,
                    color: riskColor(selectedAccount.risk_score),
                    fontWeight: 900,
                    lineHeight: 1,
                  }}
                >
              ...
              >
                <div
                  style={{
                    ...
                    height: "100%",
                    background: riskColor(selectedAccount.risk_score),
                    borderRadius: 3,
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                ...
                >
                  <div
                    style={{ fontSize: 9, color: "#475569", marginBottom: 2 }}
                  >
                    TRANSACTIONS
                  </div>
                  <div
                    style={{ fontSize: 16, fontWeight: 700, color: "#e6edf3" }}
                  >
                    {selectedTxns.length}
                  </div>
                </div>
            ...
            >
              ...
              {selectedAccount.reasons?.length > 0 ? (
                ...
                >
                  <Eye size={12} style={{ color: "#475569" }} /> No specific
                  flags — ML anomaly detected
                </div>
              )}
            </div>
