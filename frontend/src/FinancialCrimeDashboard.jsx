...
import {
  ...
  List, ArrowUpDown, ArrowUp, ArrowDown, Crosshair, Route,
} from 'lucide-react';

const API_BASE = 'https://nexora-ds7m.onrender.com';
...
// Animated count-up number, used for the risk score in the side panel.
function CountUp({ value, duration = 700, style }) {
...
// Shortest hop path between two accounts, undirected, BFS over the transaction graph.
function findPath(fromId, toId, transactions) {
...
export default function FinancialCrimeDashboard() {
  ...
  const [introPhase, setIntroPhase] = useState('scanning');
  ...
  const traceFromRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // Ambient particles: 1-2 per link, drifting from source to target continuously.
    particlesRef.current = links.flatMap((l) => {
      const n = l.amount > 50000 ? 2 : 1;
      return Array.from({ length: n }, () => ({
        link: l, t: Math.random(), speed: 0.0025 + Math.random() * 0.0025,
      }));
    });
  ...
  // Cinematic intro sequence — runs once, the first time accounts load.
  useEffect(() => {
    if (!accounts.length || introDoneRef.current) return;
    introDoneRef.current = true;
    introStartRef.current = Date.now();
    setIntroPhase('scanning');
    const t1 = setTimeout(() => setIntroPhase('revealing'), 900);
    const t2 = setTimeout(() => setIntroPhase('done'), 2400);
  ...
  useEffect(() => {
    ...
    function handleClick(e) {
      ...
      if (traceModeRef.current) {
        if (!n) return;
        if (traceFromRef.current === null) {
          traceFromRef.current = n.id;
          setTraceFrom(n.id);
        } else if (n.id !== traceFromRef.current) {
          const path = findPath(traceFromRef.current, n.id, transactions);
          tracePathRef.current = path;
          setTracePath(path);
          traceStartRef.current = Date.now();
          traceModeRef.current = false;
          setTraceMode(false);
          traceFromRef.current = null;
          setTraceFrom(null);
        }
        return;
      }
    ...
    function frame() {
      ...
      // Ambient money-flow particles drifting along every edge.
      for (const p of particlesRef.current) {
        p.t += p.speed;
        if (p.t > 1) p.t -= 1;
        const s = p.link.source, tg = p.link.target;
        if (!s || !tg || s.x == null || tg.x == null) continue;
        const px = s.x + (tg.x - s.x) * p.t;
        const py = s.y + (tg.y - s.y) * p.t;
        const isHighRisk = (s.risk_score > HIGH_RISK_THRESHOLD) && (tg.risk_score > HIGH_RISK_THRESHOLD);
        ctx.beginPath(); ctx.arc(px, py, isHighRisk ? 1.6 : 1.1, 0, Math.PI * 2);
        ctx.fillStyle = isHighRisk ? 'rgba(240,180,41,0.55)' : 'rgba(148,163,184,0.3)';
        ctx.fill();
      }

      // Nodes
      const now = Date.now();
      const introElapsed = introStartRef.current ? now - introStartRef.current : 9999;
      const introNodeAlpha = Math.max(0, Math.min(1, (introElapsed - 700) / 900));
      for (const n of nodes) {
        ...
        ctx.globalAlpha = dim ? 0.25 : 1 * introNodeAlpha;
        ...
        if (traceModeRef.current && n.id === traceFromRef.current) {
          ctx.globalAlpha = 1;
          ctx.beginPath(); ctx.arc(n.x, n.y, n.r + 7, 0, Math.PI * 2);
          ctx.strokeStyle = '#f0b429'; ctx.lineWidth = 2 / t.k; ctx.stroke();
        }
      ...
      // Trace the Money — animated pulse traveling along the resolved path.
      const path = tracePathRef.current;
      if (path && path.length > 1) {
        const byId = new Map(nodes.map(n => [n.id, n]));
        const elapsed = traceStartRef.current ? (Date.now() - traceStartRef.current) / 1000 : 0;
        const segDuration = 0.55;
        const totalDuration = (path.length - 1) * segDuration;
        const loopT = totalDuration > 0 ? (elapsed % (totalDuration + 0.6)) : 0;

        // Draw the full path as a glowing gold line first.
        ctx.beginPath();
        let started = false;
        ...
        for (const id of path) {
          ...
          ctx.strokeStyle = '#f0b429'; ctx.lineWidth = 1.5 / t.k; ctx.stroke();
        }

        // Traveling glow pulse along the path.
        if (loopT < totalDuration) {
          const segIdx = Math.min(path.length - 2, Math.floor(loopT / segDuration));
          const segT = (loopT - segIdx * segDuration) / segDuration;
          const a = byId.get(path[segIdx]);
          const b = byId.get(path[segIdx + 1]);
          if (a && b) {
            const px = a.x + (b.x - a.x) * segT;
            const py = a.y + (b.y - a.y) * segT;
            ctx.shadowColor = '#f0b429'; ctx.shadowBlur = 22;
            ctx.beginPath(); ctx.arc(px, py, 5 / Math.sqrt(t.k), 0, Math.PI * 2);
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        @keyframes introBar { 0% { transform: translateX(-100%); } 100% { transform: translateX(350%); } }
      ...
      <div style={{ flex: 1, minHeight: 0, position: 'relative', display: 'flex' }}>
        ...
        {activeTab === 'graph' && (
          <div ref={containerRef} style={{ flex: 1, position: 'relative' }}>
            ...
            {introPhase !== 'done' && !loading && !error && (
              <div style={{
                position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
                pointerEvents: 'none', flexDirection: 'column', gap: 10,
                opacity: introPhase === 'revealing' ? 0 : 1, transition: 'opacity 0.6s ease'
              }}>
                <div className="font-display" style={{
                  fontSize: 13, letterSpacing: '0.3em', color: '#f0b429', textTransform: 'uppercase',
                  animation: 'scanline 1.1s ease-in-out infinite'
                }}>
                  ANALYZING {accounts.length} ACCOUNTS...
                </div>
                <div style={{ width: 220, height: 2, background: '#1c2330', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ width: '40%', height: '100%', background: '#f0b429', animation: 'introBar 0.9s ease-in-out infinite' }} />
                </div>
              </div>
            )}

            {/* Trace the Money control */}
            {!loading && !error && (
              <button
                onClick={() => {
                  ...
                  setTraceFrom(null);
                  setTracePath(null);
                  tracePathRef.current = null;
                }}
                style={{
                  position: 'absolute', top: 16, right: 16, zIndex: 10,
                  background: traceMode ? 'rgba(240,180,41,0.18)' : 'rgba(6,10,18,0.85)',
                  border: `1px solid ${traceMode ? '#f0b429' : '#1c2330'}`,
                  borderRadius: 8, padding: '8px 12px', cursor: 'pointer',
                  color: traceMode ? '#f0b429' : '#94a3b8', fontSize: 10,
                  fontFamily: 'inherit', letterSpacing: '0.1em', display: 'flex', alignItems: 'center', gap: 6,
                  backdropFilter: 'blur(8px)'
                }}>
                <Route size={13} />
                {traceMode ? (traceFrom ? 'SELECT DESTINATION...' : 'SELECT SOURCE...') : 'TRACE THE MONEY'}
              </button>
            )}
            {tracePath && (
              <div style={{
                position: 'absolute', top: 60, right: 16, zIndex: 10,
                background: 'rgba(6,10,18,0.9)', border: '1px solid #1c2330', borderRadius: 8,
                padding: '8px 12px', fontSize: 10, color: '#cbd5e1', maxWidth: 220, backdropFilter: 'blur(8px)'
              }}>
                <div style={{ color: '#f0b429', letterSpacing: '0.1em', marginBottom: 4 }}>
                  {tracePath.length - 1} HOP{tracePath.length > 2 ? 'S' : ''} TRACED
                </div>
                {tracePath.map(id => accountsById.get(id)?.name || id).join(' → ')}
                <button onClick={() => { setTracePath(null); tracePathRef.current = null; }}
                  style={{ display: 'block', marginTop: 6, background: 'none', border: 'none', color: '#475569', fontSize: 9, cursor: 'pointer', padding: 0, letterSpacing: '0.1em' }}>
                  CLEAR ✕
                </button>
              </div>
            )}
            ...
            <div style={{ padding: '14px 18px', borderBottom: '1px solid #1c2330' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <span className="font-display" style={{ fontSize: 32, color: riskColor(selectedAccount.risk_score), fontWeight: 900, lineHeight: 1 }}>
                  <CountUp value={selectedAccount.risk_score} />
                </span>
              </div>
